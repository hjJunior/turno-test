import ApproveButton from "@/modules/admins/components/PendingCheckDepositRow/ApproveButton.vue";
...
describe("ApproveButton", () => {
  ...
  given("button", () => screen.getByRole("button", { name: "Approve" }));
  ...
  describe("when confirms", () => {
    it("can approve", async () => {
      ...
      expect(api.post).toHaveBeenCalledWith("/api/check-deposits/21/accept");
    });
  });
